...
contract Voting {
    ...
    mapping(string => bool) public usedVoterIds; // Prevent duplicate voter IDs
    ...
    function registerVoter(
        ...
        require(!usedVoterIds[_voterId], "Duplicate voterId not allowed");
    ...
    function retrieveVotes() public view returns (Candidate[] memory) {
        require(block.timestamp > votingEnd, "Election is still ongoing. Results will be available after it ends.");
        return candidates;
    }
    ...
    function resetElection() public onlyOwner {
        ...
        for (uint256 i = 0; i < listOfVoters.length; i++) {
            ...
            string memory voterId = voterDetails[voter].voterId;

            delete uniqueIdToAddress[uniqueId];
            delete usedVoterIds[voterId];
